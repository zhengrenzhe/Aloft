...
停止命令时，Aloft 向整个进程组发送 `SIGTERM`，并等待最多 5 秒。进程组仍未退出时，Aloft 会显示原生确认框并列出命令名和 PGID；取消会保留原进程，确认后才向超时操作捕获的进程组发送 `SIGKILL` 并验证进程组已经消失。Restart 只会在验证完成后启动新一代进程。退出 Aloft 时仍只使用 `SIGTERM`，不会自动强制停止进程。
